  ...
  try {
    ...
    if(type === "category") {
     ...
     body = JSON.stringify(recipes.map(r=>({name:r.name,emoji:r.emoji||"🍽️",time:r.time||"30 min",diff:r.diff||"Medium",cal:r.cal||320,protein:r.protein||"12g",tags:r.tags||[],category:r.category||"General"})));
      ...
      headers["Prefer"] = "return=minimal";
    } else if(type === "get_grocery_cache") {
      url = `${supabaseUrl}/rest/v1/grocery_cache?recipe_name=eq.${encodeURIComponent(recipe_name)}&limit=1`;
    } else if(type === "save_grocery_cache") {
      url = `${supabaseUrl}/rest/v1/grocery_cache`;
      method = "POST";
